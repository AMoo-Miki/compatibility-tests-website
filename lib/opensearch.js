...
const getLatestRefs = async (key, src) => {
  const project = keyProjects[key];
  const result = await getAggregates(summariesIndex, {
    ...
    query: {
      bool: {
        filter: [
          { term: { [`package.${project.slug}.src`]: src || project.src } },
          { term: { 'scope': key } },
        ]
      }
    }
...
const getRefsCounts = async (key, refs, security, detailed) => {
  ...
  const detailsQuery = detailed ? {
      detail: {
        top_hits: {
          sort: [{ 'timestamp': { order: 'desc' } }],
          _source: {
            includes: ['timestamp', `package.${project.slug}.version`]
          },
          size: 1
        }
      }
  ...
  if (result.detail?.hits.hits[0]) {
    ...
    detail.packageVersion = resultDetail.package[project.slug].version;
  }
